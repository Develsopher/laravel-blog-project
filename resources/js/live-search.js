  ...
  renderResultsHTML(posts) {
    ...
    if (posts.length) {
      ...
      ${posts
        ...
        <img class="avatar-tiny" src="/storage/avatars/${post.user.avatar}"> <strong>${post.title}</strong>
        <span class="text-muted small">by ${post.user.name} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
